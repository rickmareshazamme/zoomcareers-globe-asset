...
window.addEventListener('load', function () {
    if (globeLoaded && !sliderInitialized) {
    ...
    locations.forEach((location, idx) => {
        ...
        sliderItem.innerHTML = `
            <div class="slider-image-wrapper">
                ...
                <div class="slider-location-name"><img src="${location.countryIconURL}" alt="${location.name}" />${location.name}</div>
            </div>
        `;
...
function initScene() {
    ...
    orbit.minPolarAngle = 0.36 * Math.PI;
...
function cretePointers() {
    const pointerGeometry = new THREE.IcosahedronGeometry(0.1, 3);
...
function render() {
    ...
    locations.forEach(l => {
        ...
        const x = (activePointPositionProjected.x + 1) * popupsContainer.offsetWidth * 0.5;
        const y = (1 - activePointPositionProjected.y) * popupsContainer.offsetHeight * 0.5;
        ...
        l.midScreenDist = Math.abs(x - popupsContainer.offsetWidth * 0.5);
        if (l.popup.style.opacity < 0.1) {
            l.midScreenDist = 10000;
        }
        l.popup.style.display = l.popup.style.opacity < 0.15 ? "none" : "block";
    });
